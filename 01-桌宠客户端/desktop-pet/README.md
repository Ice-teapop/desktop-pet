...
> 桌面上的芙宁娜 chibi 桌宠陪你写代码 — Electron + React + TypeScript 透明置顶桌宠，背后是多 provider AI 助手（Anthropic / OpenAI / Google / xAI / DeepSeek / 字节豆包）。

当前版本 **v0.4.17**。当前活跃主题是 [deskpet-furina](themes/deskpet-furina/)：Furina chibi SVG/SMIL 主题，包含 15 个常规状态、3 个过渡、2 个反应和 6 个 mini 状态。

---

## 给普通用户：安装

当前 Furina fork 以本地开发和 release 包安装为准；仓库内没有旧主线的 `scripts/install.sh` 一键安装脚本。

到 Releases 下载对应平台包：

- Apple Silicon：`DeskPet-Furina-<version>-arm64-mac.zip`
- Intel Mac：`DeskPet-Furina-<version>-mac.zip`
- Windows：`DeskPet-Furina-<version>-setup.exe`
- Linux：`DeskPet-Furina-<version>-x86_64.AppImage` / `_amd64.deb`
- 英文版加 `-EN` 前缀

macOS 手动安装的话首次需要右键 → 打开（绕 Gatekeeper，没 Apple 公证），或在终端跑 `xattr -dr com.apple.quarantine /Applications/DeskPet-Furina.app`。

---

## 首次启动

启动后桌面右下角出现 Furina 桌宠 + 一个空对话框。引导你：
...
2. 系统弹「DeskPet-Furina 想录制屏幕」权限请求 → 允许
3. 没弹的话：**系统设置 → 隐私与安全性 → 屏幕录制** → 加 `DeskPet-Furina.app`（dev 模式下是 `node_modules/electron/dist/Electron.app`）
4. **完全退出 DeskPet-Furina 重启**（macOS 权限传播要求）
...
- **文件操作审计**：本地 `~/Library/Application Support/DeskPet-Furina/audit.log`（JSONL，5MB 自动滚动），不上传
- **聊天历史**：本地 `~/Library/Application Support/DeskPet-Furina/chat-history.json`，不同步任何云
...
- Furina SVG/SMIL 主题：15 个常规状态 + 3 个过渡 + 2 个反应 + 6 个 mini 状态
...
- chat history 持久化（chat-history.json）+ pet memory（pet-memory.md）+ user profile
...
# 4. publish 为 latest
gh release edit v0.4.17 --draft=false --latest
```
...
- 开源了 [pet-forge](https://github.com/rullerzhou-afk/pet-forge)（MIT）这套 SVG 桌宠制作工作流，本项目用它搭建 [deskpet-furina 主题](themes/deskpet-furina/) 的 Furina SVG/SMIL 状态资产
- 她的 [Clawd 项目](https://github.com/rullerzhou-afk/clawd-on-desk) 为本项目提供了**骨架层面的学习参考**（动画 rigging 思路 / 状态机层次结构）

### ⚠️ 事实边界声明

- **Clawd 形象本身未获原作者授权使用** — 本项目早期临时借用过 Clawd 资产，被原作者发现后她给了一个缓冲期让我重做；当前 Furina fork 不依赖 Clawd 资产
- deskpet-furina SVG/SMIL 资产独立维护，**不含任何 Clawd 资产**
