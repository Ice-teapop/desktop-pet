...
import { PET_ANIMATIONS, isPetAnimation, type PetAnimation } from '../../shared/pet-state'
...
export type ToolResult = { ok: true; content: ToolResultContent } | { ok: false; error: string }
  ...
  input_schema: {
    ...
    properties: {
      ...
      paths: {
        type: 'array',
        items: { type: 'string' },
        description: 'Batch paths (absolute or ~/-relative). Preferred for multi-file deletion.'
      }
    },
...
function execSetPetAnimation(input: unknown, ctx: ToolContext): ToolResult {
  ...
  return {
    ok: true,
    content: `Pet started ${name} animation (auto-returns to idle after one cycle).`
  }
}
...
async function execRemember(input: unknown): Promise<ToolResult> {
  ...
  try {
    ...
    return {
      ok: true,
      content: `Remembered. (Persisted to ${path.join(app.getPath('userData'), 'pet-memory.md')})`
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
 ...
 */
function wrapUntrusted(source: string, attrs: Record<string, string>, content: string): string {
  ...
  const safe = content.replace(/<\s*\/\s*external_content\s*>/gi, '<\\/external_content>')
...
async function execReadFile(input: unknown, ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    const isWebp = magicHex.startsWith('52494646') && sniff.subarray(8, 12).toString() === 'WEBP'
    ...
    if (isPng || isJpeg || isGif || isWebp) {
      ...
      try {
        ...
        const mediaType: 'image/png' | 'image/jpeg' | 'image/gif' | 'image/webp' = isPng
          ? 'image/png'
          : isJpeg
            ? 'image/jpeg'
            : isGif
        ...
        return {
          ...
          content: [
            {
              type: 'text',
              text: `[图片: ${gate.absPath} (${(buf.length / 1024).toFixed(0)}KB ${mediaType})]`
            },
            {
              type: 'image',
...
async function execWriteFile(input: unknown): Promise<ToolResult> {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!safety.ok) {
      ...
      return {
        ok: false,
        error: `files[${i}] 路径被黑名单拦 (整批拒): ${it.path} (${safety.reason})`
      }
    }
    const trust = checkTrusted(safety.absPath)
...
function containsCjk(s: string): boolean {
  // CJK Unified Ideographs + extensions + common full-width punctuation.
...
async function execWritePdf(input: unknown): Promise<ToolResult> {
  ...
  const fontSize =
    typeof obj.fontSize === 'number' && obj.fontSize >= 6 && obj.fontSize <= 72 ? obj.fontSize : 12
  ...
  const gate = await requestPathApprovalWithPreview(obj.path, 'write_pdf', '写 PDF 文档', preview)
  if (!gate.ok) return gate
  try {
    ...
    if (needCjk && !cjkFontRegistered) {
      return {
        ok: false,
        error:
          '内容含中文但系统找不到 CJK 字体 (PingFang/Hiragino/STHeiti/Songti 都没装). 用 write_docx 代替.'
      }
    }
...
async function execDeleteFile(input: unknown): Promise<ToolResult> {
  ...
  const blacklistHits = safetyResults.map((s, i) => ({ s, i })).filter(({ s }) => !s.ok)
  ...
  for (const absPath of safePaths) {
    try {
      ...
      try {
        ...
        failed.push({
          path: absPath,
          err: `trash failed (${msg}); hard-delete also failed: ${msg2}`
        })
        await logToolAction({
          tool: 'delete_file',
          argsSummary: `batch_id=${batchId} path=${absPath}`,
  ...
  const buildOkLines = (): string => {
    ...
    if (hardDeleted.length > 0)
      lines.push(`Hard-deleted (trash unavailable):\n${hardDeleted.join('\n')}`)
    return lines.join('\n')
  }
  if (failed.length === 0) {
    return {
...
async function resolveMovePair(
  ...
  rawDest: string,
  overwrite: boolean
): Promise<{ ok: true; srcAbs: string; finalDest: string } | { ok: false; error: string }> {
...
async function execMoveFile(input: unknown): Promise<ToolResult> {
  ...
  if (Array.isArray(obj.moves)) {
    for (let i = 0; i < obj.moves.length; i++) {
      ...
      if (typeof mo.dest !== 'string')
        return { ok: false, error: `moves[${i}].dest must be string` }
  ...
  const allTrusted = resolved.every((r) => checkTrusted(r.srcAbs) && checkTrusted(r.finalDest))
...
async function execCopyFile(input: unknown): Promise<ToolResult> {
  ...
  if (Array.isArray(obj.copies)) {
    for (let i = 0; i < obj.copies.length; i++) {
      ...
      if (typeof co.dest !== 'string')
        return { ok: false, error: `copies[${i}].dest must be string` }
  ...
  if (failed.length === 0) {
    return {
      ok: true,
      content: isBatch ? `Copied ${copied.length}:\n${copied.join('\n')}` : copied[0]
    }
  }
  return {
    ok: true,
...
async function execListDirectory(input: unknown): Promise<ToolResult> {
  ...
  try {
    ...
    const lines: string[] = [
      `# ${gate.absPath}`,
      `# ${entries.length} entries (showing ${limited.length})`
    ]
...
const SETTINGS_PANE_URL: Record<string, string> = {
  ...
  privacy_full_disk: 'x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles',
...
async function execOpenSystemSettings(input: unknown): Promise<ToolResult> {
  ...
  if (!url) {
    return {
      ok: false,
      error: `unknown pane: ${pane}. supported: ${Object.keys(SETTINGS_PANE_URL).join(', ')}`
    }
  }
  try {
 ...
 */
async function checkHostSafety(
  hostname: string
...
async function execGetWeather(input: unknown): Promise<ToolResult> {
  ...
  try {
    ...
    if (!geoResp.ok) {
      ...
      return {
        ok: false,
        error: `geocoding HTTP ${geoResp.status}: 服务暂不可用，告知用户稍后再试`
      }
    }
    ...
    if (!geoJson.results || geoJson.results.length === 0) {
      ...
      return {
        ok: false,
        error: `没找到城市 "${location}"，换个名字试 (e.g. "Beijing" / "北京市")`
      }
    }
...
async function execWebSearch(input: unknown, ctx: ToolContext): Promise<ToolResult> {
  if (!ctx.tavilyApiKey) {
    return {
      ok: false,
      error: 'Tavily API key 未配置 —— 设置 TAVILY_API_KEY 环境变量或在设置面板填'
    }
  }
  if (typeof input !== 'object' || input === null) {
